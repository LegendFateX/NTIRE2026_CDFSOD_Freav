...
for model in "${model_list[@]}"; do
  for dataset in "${data_list[@]}"; do
    for shot in ${shot_list[@]}; do
      CUDA_VISIBLE_DEVICES=0,1,2,3 python3 ./tools/extract_instance_prototypes.py   --dataset ${dataset}_${shot}shot --out_dir prototypes_init --model vit${model}14 --epochs 1 --use_bbox yes --without_mask True
      echo "extract_instance_prototypes with vit${model} for ${shot}shot ${dataset} done, save at prototypes_init dir."
      CUDA_VISIBLE_DEVICES=0,1,2,3 python3 ./tools/run_sinkhorn_cluster.py  --inp  prototypes_init/${dataset}_${shot}shot.vit${model}14.bbox.pkl  --epochs 30 --momentum 0.002    --num_prototypes ${shot}
